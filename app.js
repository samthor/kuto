...
function createSequenceExpression(...parts2) {
  return {
...
function createExpressionStatement(...body) {
  return {
...
function processPattern(p) {
  ...
  while (pending.length) {
    const p2 = pending.shift();
    switch (p2.type) {
      case "Identifier":
        names.push(p2.name);
        continue;
      case "RestElement":
        pending.push(p2.argument);
        continue;
      case "ArrayPattern":
        for (const e2 of p2.elements) {
          e2 && pending.push(e2);
        }
        continue;
      case "AssignmentPattern":
        pending.push(p2.left);
        expr.push(p2.right);
        continue;
      case "ObjectPattern":
        for (const prop of p2.properties) {
        ...
        continue;
      case "MemberExpression":
        if (p2.object.type !== "Super") {
          expr.push(p2.object);
        }
        if (p2.property.type !== "PrivateIdentifier") {
          expr.push(p2.property);
        }
        continue;
    }
  ...
  return {
    ...
    init: init.length ? createSequenceExpression(...init) : void 0
  };
}
function reductifyClassParts(c) {
  const e2 = [];
  c.superClass && e2.push(c.superClass);
      ...
      case "PropertyDefinition":
        ...
        if (part.value) {
          if (part.static || part.type === "MethodDefinition") {
            ...
            e2.push(createIife([expr]));
          }
        }
        break;
      case "StaticBlock":
        e2.push(createIife(part.body));
        break;
    }
  }
  return e2.length === 1 ? e2[0] : createSequenceExpression(...e2);
}
function reductifyFunction(f) {
  const body = [];
...
function reductifyStatement(b) {
  switch (b.type) {
    ...
    case "IfStatement": {
      const body = [createExpressionStatement(b.test), b.consequent];
    ...
    case "SwitchStatement": {
      const body = [createExpressionStatement(b.discriminant)];
...
function analyzeBlock(b) {
  const out2 = { vars: /* @__PURE__ */ new Map() };
  const markIdentifier = (name, written = false) => {
    const prev = out2.vars.get(name);
    if (prev) {
    ...
    out2.vars.set(name, info);
    return info;
  };
  const processExpression = (e2) => {
    switch (e2.type) {
      ...
      case "ChainExpression":
      case "ParenthesizedExpression":
        processExpression(e2.expression);
        break;
      case "SpreadElement":
      case "YieldExpression":
      case "AwaitExpression":
      case "UnaryExpression":
        e2.argument && processExpression(e2.argument);
        break;
      case "Identifier":
        markIdentifier(e2.name);
        break;
      case "AssignmentExpression": {
        const p = processPattern(e2.left);
        p.names.forEach((name) => markIdentifier(name, true));
        p.init && processExpression(p.init);
        processExpression(e2.right);
        break;
      }
      case "UpdateExpression": {
        if (e2.argument.type === "Identifier") {
          markIdentifier(e2.argument.name, true);
        } else {
          processExpression(e2.argument);
        }
        break;
      }
      case "NewExpression":
      case "CallExpression":
        e2.arguments.forEach((arg) => processExpression(arg));
        if (e2.callee.type === "ArrowFunctionExpression" || e2.callee.type === "FunctionExpression" && !e2.callee.id) {
          const block = reductifyFunction(e2.callee);
          ...
          processExpression(e2.callee);
        }
        break;
      case "TemplateLiteral":
      case "SequenceExpression":
        e2.expressions.forEach((arg) => processExpression(arg));
        break;
      case "ArrayExpression":
        e2.elements.forEach((el) => el && processExpression(el));
        break;
      case "ConditionalExpression":
        processExpression(e2.test);
        processExpression(e2.consequent);
        processExpression(e2.alternate);
        break;
      case "BinaryExpression":
      case "LogicalExpression":
        processExpression(e2.left);
        processExpression(e2.right);
        break;
      case "ClassExpression":
        processExpression(reductifyClassParts(e2));
        break;
      case "FunctionExpression":
      case "ArrowFunctionExpression": {
        const block = reductifyFunction(e2);
        const inner = analyzeBlock(block);
        for (const [key, info] of inner.vars) {
          if (info.kind) {
            continue;
          }
          const prev = out2.vars.get(key);
      ...
      case "TaggedTemplateExpression":
        processExpression(e2.tag);
        processExpression(e2.quasi);
        break;
      case "MemberExpression":
        processExpression(e2.object);
        e2.computed && processExpression(e2.property);
        break;
      case "ObjectExpression": {
        for (const prop of e2.properties) {
      ...
      case "ImportExpression":
        processExpression(e2.source);
        break;
      default:
        throw new Error(`should not get here: ${e2.type}`);
    }
  };
...
// lib/helper.ts
function withDefault(m, k, build) {
  if (m.has(k)) {
    return m.get(k);
  }
...
function safeImportAs(from, to = from) {
  if (from === to) {
    return from;
  }
  return `${from} as ${to}`;
...
var ModDef = class {
  constructor() {
    ...
    this.byLocalName = /* @__PURE__ */ new Map();
    this._exports = /* @__PURE__ */ new Map();
    this.allLocalExported = /* @__PURE__ */ new Map();
  }
  /**
  ...
  *localExported() {
  ...
  _addImport(importSource, localName, remoteName) {
    ...
    const s = withDefault(info.imports, remoteName, () => /* @__PURE__ */ new Set());
    s.add(localName);
  }
  addGlobalImport(importSource, localName) {
    return this._addImport(importSource, localName, "");
  }
  addImport(importSource, localName, remoteName = localName) {
    if (remoteName === "") {
      throw new Error(`can't have blank remoteName`);
    }
    return this._addImport(importSource, localName, remoteName);
  }
  removeImport(localName) {
    const prev = this.byLocalName.get(localName);
    if (!prev) {
      return false;
    }
    ...
    const s = withDefault(info.imports, prev.remote, () => /* @__PURE__ */ new Set());
    s.delete(localName);
    if (s.size === 0) {
  ...
  addExportFrom(importSource, exportedName, remoteName = "") {
    ...
    const info = this.addSource(importSource);
    const s = withDefault(info.exports, remoteName, () => /* @__PURE__ */ new Set());
    s.add(exportedName);
  }
  markExportAllFrom(importSource) {
    const info = this.addSource(importSource);
    info.reexportAll = true;
  }
  addExportLocal(exportedName, sourceName2 = exportedName) {
    const prev = this._exports.get(exportedName);
    if (prev) {
      if (prev.import || prev.name !== sourceName2) {
        throw new Error(`already exported: ${exportedName}`);
      }
      return;
    }
    const p = { name: sourceName2 };
    this._exports.set(exportedName, p);
    this.allLocalExported.set(exportedName, p);
  }
  removeExportLocal(exportedName) {
    const prev = this._exports.get(exportedName);
    if (!prev) {
      return false;
    }
  ...
  renderSource() {
    const lines = [];
    for (const [path3, info] of this.bySource) {
      const pj = JSON.stringify(relativize(path3));
      let any = false;
      ...
      for (const [remote, local] of info.imports) {
        ...
        for (const l of local) {
          parts2.push(safeImportAs(remote, l));
        }
      }
      if (parts2.length) {
        lines.push(`import { ${parts2.join(", ")} } from ${pj}`);
        any = true;
      }
      if (info.reexportAll) {
      ...
      for (const [remote, exported] of info.exports) {
        for (const e2 of exported) {
          reexportParts.push(safeImportAs(remote, e2));
        }
      }
    ...
    if (this.allLocalExported.size) {
      ...
      for (const [remote, { name }] of this.allLocalExported) {
        parts2.push(safeImportAs(name, remote));
      }
      lines.push(`export { ${parts2.join(", ")} }`);
    }
    lines.push("");
    return lines.join(";\n");
  }
...
var fakeDefaultIdentifier = Object.freeze({
  start: -1,
  end: -1,
  type: "Identifier",
  name: "default"
});
var buildFakeDefaultConst = (expr) => {
  const decl = {
    start: -1,
    end: -1,
    type: "VariableDeclarator",
...
function nodeToString(source2) {
  if (typeof source2 === "string") {
    return source2;
  }
  if (source2.type === "Identifier") {
    return source2.name;
  }
  if (typeof source2.value === "string") {
    return source2.value;
  }
  throw new Error(`importing non-string?`);
}
function aggregateImports(p) {
  const out2 = {
    mod: new ModDef(),
    localConst: /* @__PURE__ */ new Set(),
    rest: [],
    moduleNodes: []
  };
    ...
    switch (node.type) {
      ...
      case "ExportNamedDeclaration":
        if (node.source) {
          ...
          out2.mod.addSource(importSource);
        }
        break;
  ...
  for (const node of p.body) {
    switch (node.type) {
        ...
        for (const s of node.specifiers) {
          ...
          switch (s.type) {
            case "ImportNamespaceSpecifier":
              out2.mod.addGlobalImport(importSource, local);
              break;
            case "ImportDefaultSpecifier":
              out2.mod.addImport(importSource, local, "default");
              break;
            case "ImportSpecifier":
              out2.mod.addImport(importSource, local, nodeToString(s.imported));
              break;
          }
        }
        break;
      ...
      case "ExportAllDeclaration":
        out2.mod.markExportAllFrom(node.source.value);
        continue;
      case "ExportNamedDeclaration":
        if (!node.declaration) {
          const names = [];
          ...
          if (node.source) {
            ...
            for (const { exportedName, name } of names) {
              out2.mod.addExportFrom(importSource, exportedName, name);
            }
          } else {
            for (const { exportedName, name } of names) {
              out2.mod.addExportLocal(exportedName, name);
            }
          }
          continue;
          ...
          for (const s of node.declaration.declarations) {
            const p2 = processPattern(s.id);
            p2.names.forEach((name) => {
              out2.mod.addExportLocal(name);
              isConst && out2.localConst.add(name);
            });
          }
          out2.rest.push(node.declaration);
          continue;
        }
      case "ExportDefaultDeclaration": {
        ...
        switch (d.type) {
          ...
          default:
            out2.mod.addExportLocal("default", "default");
            out2.localConst.add("default");
            out2.rest.push(buildFakeDefaultConst(d));
            continue;
        }
        if (d.id && isDefault) {
          out2.mod.addExportLocal("default", d.id.name);
        } else if (d.id) {
          out2.mod.addExportLocal(d.id.name, d.id.name);
        } else if (isDefault) {
          out2.mod.addExportLocal("default", "default");
          out2.localConst.add("default");
        } else {
        ...
        out2.rest.push({ ...d, id: d.id || fakeDefaultIdentifier });
        continue;
      }
      case "VariableDeclaration":
        out2.rest.push(node);
        ...
        for (const s of node.declarations) {
          const p2 = processPattern(s.id);
          p2.names.forEach((name) => out2.localConst.add(name));
        }
        continue;
      default:
        out2.rest.push(node);
...
function analyzeFunction(f) {
  ...
  const out2 = { external: /* @__PURE__ */ new Map() };
  for (const [key, info] of internal.vars) {
    if (info.kind) {
      continue;
    }
    out2.external.set(key, info.written || info.nestedWrite);
  }
  return out2;
}

// lib/extractor.ts
var normalStaticPrefix = "_";
var callableStaticPrefix = "$";
function extractExistingStaticCode(raw) {
  const existingByCode = /* @__PURE__ */ new Map();
  for (const [path3, source2] of raw) {
    const p = parse3(source2, { ecmaVersion: "latest", sourceType: "module" });
    const agg = aggregateImports(p);
    const add = (node, name) => {
      const code = source2.substring(node.start, node.end);
      if (!existingByCode.has(code)) {
        existingByCode.set(code, { name, import: path3 });
      }
    };
...
var StaticExtractor = class {
  constructor(args2) {
    this.staticToWrite = /* @__PURE__ */ new Map();
    this.staticVars = /* @__PURE__ */ new Set();
    this.nodesToReplace = /* @__PURE__ */ new Map();
    this.args = { ...args2 };
    this.existingByCode = extractExistingStaticCode(args2.existingStaticSource);
    for (const info of this.existingByCode.values()) {
      this.staticVars.add(`${info.name}~${info.import}`);
    }
    const p = parse3(args2.source, { ecmaVersion: "latest", sourceType: "module" });
    const agg = aggregateImports(p);
    this._block = createBlock(...agg.rest);
    const analysis = analyzeBlock(this._block);
    this.agg = agg;
    this.vars = analysis.vars;
    for (const p2 of this._block.body) {
      switch (p2.type) {
        case "ClassDeclaration":
        case "FunctionDeclaration":
          if (p2.id.name === "default") {
   ...
   */
  varForStatic(staticName2, prefix) {
    for (let i = 1; i < 1e4; ++i) {
      const cand = `${prefix}${i.toString(36)}`;
      const check = `${cand}~${staticName2}`;
      if (!this.staticVars.has(check)) {
        this.staticVars.add(check);
        return cand;
      }
    }
    throw new Error(`could not make var for static: ${staticName2}`);
  }
  varForMain(prefix = "$") {
    for (let i = 1; i < 1e4; ++i) {
      const cand = `${prefix}${i.toString(36)}`;
  ...
  addCodeToStatic(args2) {
    const find = this.findVars(args2.find);
    if (find.rw) {
      return null;
    }
    if (!args2.decl && find.immediateAccess) {
      throw new Error(`top-level fn should not have immediateAccess`);
    }
    let name = "";
    let targetStaticName = this.args.staticName;
    let code = this.args.source.substring(args2.node.start, args2.node.end);
    const existing2 = this.existingByCode.get(code);
    if (existing2) {
      targetStaticName = existing2.import;
      name = existing2.name;
    }
    if (!name) {
      if (args2.decl) {
      ...
      } else if ("id" in args2.node) {
        const id = args2.node.id;
    ...
    if (!name || name === "default") {
      throw new Error(`could not name code put into static: ${args2}`);
    }
    ...
    if (args2.decl) {
      const replacedCode = `${targetStatic.globalInMain}.${name}` + (find.immediateAccess ? "()" : "");
      this.nodesToReplace.set(args2.node, replacedCode);
      this.agg.mod.addGlobalImport(targetStaticName, targetStatic.globalInMain);
    } else {
      this.nodesToReplace.set(args2.node, "");
  ...
  liftFunctionDeclaration(fn) {
    const vi = this.vars.get(fn.id.name);
    if (vi === void 0 || !vi.simple) {
      return null;
    }
  ...
  liftExpression(e2) {
    const { vars } = analyzeBlock(createBlock(createExpressionStatement(e2)));
    return this.addCodeToStatic({ node: e2, find: vars, decl: true });
  }
  build() {
    const outStatic = /* @__PURE__ */ new Map();
    for (const [targetStaticName, info] of this.staticToWrite) {
      let code = info.body.map((p) => typeof p === "string" ? p : this.args.source.substring(p.start, p.end)).join("");
    ...
    const nodesToReplace = [...this.nodesToReplace];
    nodesToReplace.push(
...
function renderSkip(raw, skip) {
  const replaces = [...skip];
  replaces.sort(([{ start: a }], [{ start: b }]) => a - b);
  let out2 = raw.substring(0, replaces.at(0)?.[0].start);
  for (let i = 0; i < replaces.length; ++i) {
    out2 += replaces[i][1];
    out2 += raw.substring(replaces[i][0].end, replaces.at(i + 1)?.[0].start);
  }
  return out2;
}

// lib/lift.ts
function liftDefault(e2, minSize) {
  const stats = {
    fn: 0,
    expr: 0,
    assignment: 0,
...
function loadExisting(args2) {
  const existing2 = fs.readdirSync(args2.dist).filter((x) => /\.sjs-\w+.js$/.test(x)).toReversed().map((name) => {
    const bytes = fs.readFileSync(path.join(args2.dist, name));
    const text = dec.decode(bytes);
    return { name, bytes, text, skip: true };
  });
  const existingBySize = existing2.sort(({ bytes: a }, { bytes: b }) => b.length - a.length);
  const keepN = Math.min(args2.keep, existingBySize.length);
  for (let i = 0; i < keepN; ++i) {
    existingBySize[i].skip = false;
  }
  const out2 = /* @__PURE__ */ new Map();
  for (const e2 of existing2) {
    if (e2.skip) {
      continue;
    }
    out2.set(e2.name, e2.text);
  }
  return { existingStaticSource: out2, cand: existing2.map(({ name }) => name) };
}

// app.ts
var argsRaw = parseArgs({
  options: {
    min: {
      type: "string",
      default: "32",
...
var sourcePath = argsRaw.positionals[0];
var source = fs2.readFileSync(sourcePath, "utf-8");
var dist = argsRaw.positionals[1];
fs2.mkdirSync(dist, { recursive: true });
var now = +/* @__PURE__ */ new Date();
var parts = path2.parse(sourcePath);
var sourceName = parts.base;
var staticName = parts.name + `.sjs-${now.toString(36).padStart(8, "0")}.js`;
var existing = loadExisting({ dist, ...argsValues });
var args = {
  source,
  sourceName,
  staticName,
...
var e = new StaticExtractor(args);
var liftStats = liftDefault(e, argsValues.min);
var out = e.build();
var toRemove = existing.cand.filter((e2) => !out.static.has(e2));
var sizes = {};
sizes[sourceName] = out.main.length;
var statics = [...out.static].sort(([a], [b]) => b.localeCompare(a));
...
for (const e2 of toRemove) {
  fs2.rmSync(path2.join(dist, e2));
}
fs2.writeFileSync(path2.join(dist, sourceName), out.main);
for (const [name, code] of out.static) {
  fs2.writeFileSync(path2.join(dist, name), code);
}
console.info("Ok!");
